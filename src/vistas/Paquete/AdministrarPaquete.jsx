...
export default function StickyHeadTable() {
  ...
  const columns = [
    { field: "id_paquete", headerName: "ID", width: 200 },
...

    {
      ...
      renderCell: (params) => (
        <Button
          variant="contained"
          onClick={() => handleAdministrar(params.row.id_paquete)}
        >
          Administrar
        </Button>
      ),
  ...
  const handleAdministrar = (idExamen) => {
    ...
    navigate(`/vistaEditarPaquete?id_examen=${idExamen}`);
  };
      ...
      {cargando ? (
        ...
        <div style={{ height: 400, width: "100%" }}>
          <DataGrid
            ...
            checkboxSelection
            getRowId={(row) => row.id_paquete} // Especifica el identificador único
          />
        </div>
      )}
